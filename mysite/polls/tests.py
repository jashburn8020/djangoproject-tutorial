...
class QuestionModelTests(TestCase):
    ...
    def test_was_published_recently_with_old_question(self):
        ...
        is older than 1 day.
        """
        older_than_1_day = timezone.now() - datetime.timedelta(days=1, seconds=1)
        old_question = Question(pub_date=older_than_1_day)
    ...
    def test_was_published_recently_with_recent_question(self):
        ...
        """
        less_than_1_day = timezone.now() - datetime.timedelta(
            hours=23, minutes=59, seconds=59
        )
        recent_question = Question(pub_date=less_than_1_day)
...
class QuestionIndexViewTests(TestCase):
    """
    Test the index view, including the template returned by the view.
    """
    ...
    def test_two_past_questions(self):
        ...
        create_question("Past question 1 day", -1)
        response = self.client.get(reverse("polls:index"))
        self.assertQuerysetEqual(
            response.context["latest_question_list"],
            ["<Question: Past question 1 day>", "<Question: Past question 30 days>"],
        )
